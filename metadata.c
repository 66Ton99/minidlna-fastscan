...
GetVideoMetadataLite(const char * path, char * name)
{
	...
	void *date = malloc(20);
	struct tm *modtime = localtime(&file.st_mtime);
	strftime(date, 20, "%FT%T", modtime);
	...
	ret = sql_exec(db, "INSERT into DETAILS"
	                ...
	                path, (long long)file.st_size, file.st_mtime, "3:0:0.0",
	                date, "", "", "", "",
			name, "Creator", "Artist", "Genre", "Comment", "AVC_HD_AC3",
			"video/x-msvideo", 0);
        free(date);
	if( ret != SQLITE_OK )
	{
		DPRINTF(E_WARN, L_METADATA, "GetVideoMetadataLite sql error for %s%s!\n", path, name);

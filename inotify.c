...
add_watch(int fd, const char * path)
{
	...
	wd = inotify_add_watch(fd, path, IN_CREATE|IN_CLOSE_WRITE|IN_DELETE|IN_MOVE|IN_EXCL_UNLINK);
	if( wd < 0 )
	{
			...
			if( event->len )
			{
				          ...
				          (lstat(path_buf, &st) == 0) )
				{
					if( S_ISLNK(st.st_mode) )
					{
						...
						{
							DPRINTF(E_DEBUG, L_INOTIFY, "Call to inotify_insert_directory\n");
							inotify_insert_directory(pollfds[0].fd, esc_name, path_buf);
						}
						else
						{
							DPRINTF(E_DEBUG, L_INOTIFY, "Call to inotify_insert_file\n");
							inotify_insert_file(esc_name, path_buf);
						}
					}
				...
				{
					DPRINTF(E_DEBUG, L_INOTIFY, "The %s %s was %s | mask[%d]\n",
						(event->mask & IN_ISDIR ? "directory" : "file"),
						path_buf, (event->mask & IN_MOVED_FROM ? "moved away" : "deleted"), event->mask);
					if ( event->mask & IN_ISDIR )
					{
						DPRINTF(E_DEBUG, L_INOTIFY, "Call to inotify_remove_directory\n");
						inotify_remove_directory(pollfds[0].fd, path_buf);
					}
					else
					{
						DPRINTF(E_DEBUG, L_INOTIFY, "Call to inotify_remove_file\n");
						if (remove_watch(pollfds[0].fd, path_buf) == 0)
						{
							// Watch was removed, so this is symlink to directory
							DPRINTF(E_DEBUG, L_INOTIFY, "Call to inotify_remove_file changed to inotify_remove_directory\n");
							inotify_remove_directory(pollfds[0].fd, path_buf);
						} else
						{
